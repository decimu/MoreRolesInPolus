...
public static class MRIPModUpdater
{
  ...
  private static string GetReleasesUrl(int page) =>
    $"https://api.github.com/repos/{GitHubOwner}/{GitHubRepo}/releases?per_page={PerPage}&page={page}";

  /// <summary>
  /// リリースページのURLを取得
  /// </summary>
  public static string GetReleasePageUrl(string tag) =>
  ...
  public class ReleasedInfo
  {
    ...
    public IEnumerator CoUpdateAndShowDialog()
    {
      ...
      MetaScreen downloadWindow = MetaScreen.GenerateWindow(
        ...
        false,
        false,
        BackgroundSetting.Old,
        false
      );

      Virial.Compat.Size size;
      downloadWindow.SetWidget(
        NebulaGUIWidgetEngine.API.VerticalHolder(
          GUIAlignment.Center,
          new GUIWidget[]
          {
            ...
            new NoSGUIText(
              GUIAlignment.Center,
              NebulaGUIWidgetEngine.API.GetAttribute(AttributeAsset.OverlayContent),
      ...
      yield return CoDownloadAndInstall(
        DownloadUrl,
        (result, error) =>
        {
          success = result;
      ...
      if (success)
      {
        string message = Language
          .Translate("settings.mrip.update.message")
          .Replace("*", "")
      ...
      {
        ShowMessage(
          Language.Translate("settings.mrip.update.failed").Replace("*", ""),
          errorMessage
        );
      }
    }
      ...
      window.SetWidget(
        NebulaGUIWidgetEngine.API.VerticalHolder(
          ...
          new GUIWidget[]
          {
            new NoSGUIText(
              GUIAlignment.Center,
              NebulaGUIWidgetEngine.API.GetAttribute(AttributeAsset.OverlayTitle),
              new RawTextComponent(
                Language.Translate("settings.mrip.dialog.title").Replace("*", "")
              )
            ),
            ...
            NebulaGUIWidgetEngine.API.Button(
              GUIAlignment.Center,
              NebulaGUIWidgetEngine.API.GetAttribute(AttributeAsset.CenteredBoldFixed),
