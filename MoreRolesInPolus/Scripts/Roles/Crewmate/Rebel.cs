...
public class Rebel
  : DefinedRoleTemplate,
    DefinedSingleAbilityRole<Rebel.Ability>,
    DefinedRole,
    HasCitation
{
  private const float RebelVisionMultiplier = 1.5f;
  private static readonly BoolConfiguration OverrideTasksOption =
    NebulaAPI.Configurations.Configuration("options.role.rebel.overrideTasks", false);
  private static readonly IntegerConfiguration NumOfTasksRequiredOption =
    NebulaAPI.Configurations.Configuration(
      "options.role.rebel.numOfTasksRequired",
      (1, 15, 1),
      4,
      () => OverrideTasksOption
    );
  private static readonly BoolConfiguration UseMadmateIdentifySettingsOption =
    NebulaAPI.Configurations.Configuration("options.role.rebel.useMadmateIdentifySettings", true);
  private static readonly BoolConfiguration CanIdentifyImpostorsByTasksOption =
    NebulaAPI.Configurations.Configuration(
      "options.role.rebel.canIdentifyImpostorsByTasks",
      true,
      () => !UseMadmateIdentifySettingsOption
    );
  private static readonly IntegerConfiguration TasksRequiredForIdentifyOption =
    NebulaAPI.Configurations.Configuration(
      "options.role.rebel.tasksRequiredForIdentify",
      (1, 15, 1),
      2,
      () => !UseMadmateIdentifySettingsOption && CanIdentifyImpostorsByTasksOption
    );
  ...
  private Rebel()
    : base(
      ...
      [
        OverrideTasksOption,
        NumOfTasksRequiredOption,
        UseMadmateIdentifySettingsOption,
        CanIdentifyImpostorsByTasksOption,
        TasksRequiredForIdentifyOption,
      ],
  ...
  public static Rebel MyRole = new();
  ...
  public class Instance : RuntimeAssignableTemplate, RuntimeRole
  {
    ...
    private IEnumerable<IPlayerAbility> GetAbilities()
    {
      ...
      foreach (var SubAbility in ((IPlayerAbility)AbilityInstance).SubAbilities)
        yield return SubAbility;
    }

    string RuntimeAssignable.DisplayName =>
      AbilityInstance != null
        ? ((DefinedSingleAbilityRole<Ability>)Rebel.MyRole).GetDisplayName(AbilityInstance)
        : (Rebel.MyRole as DefinedAssignable).DisplayName;
    string RuntimeAssignable.DisplayColoredName =>
      (this as RuntimeAssignable).DisplayName.Color(Rebel.MyRole.UnityColor);
    IEnumerable<DefinedAssignable> RuntimeAssignable.AssignableOnHelp =>
      AbilityInstance != null
        ? [Rebel.MyRole, .. ((IPlayerAbility)AbilityInstance).SubAssignableOnHelp]
        : [Rebel.MyRole];
  ...
  public class Ability : AbstractPlayerUsurpableAbility, IPlayerAbility
  {
    ...
    public Ability(GamePlayer player)
      : base(player, false) { }

    void OnCheckWin(PlayerCheckWinEvent Event)
    {
      if (Event.Player != MyPlayer)
        return;
    ...
    void OnDecorateName(PlayerDecorateNameEvent Event)
    {
      ...
      if (Event.Player.AmOwner)
        return;
      if (!Event.Player.IsImpostor)
        return;
      if (!KnownImpostors.Contains(Event.Player.PlayerId))
        return;
    ...
    {
      if (Event.Role.Team != NebulaTeams.ImpostorTeam)
        return;
      if (!KnownImpostors.Contains(Event.Source.PlayerId))
        return;
    ...
    private void TryIdentifyImpostors()
    {
      ...
      if (IdentifyImpostorCount <= 0)
        return;

      while (KnownImpostors.Count < IdentifyImpostorCount)
      {
        int RequiredTasks = GetRequiredTasksForIdentifyStep(KnownImpostors.Count);
        if (MyPlayer.Tasks.CurrentCompleted < RequiredTasks)
          break;

        var CandidatePlayers = NebulaGameManager
          .Instance!.AllPlayerInfo.Where(Player =>
            Player.Role.Role.Category == RoleCategory.ImpostorRole
            && !KnownImpostors.Contains(Player.PlayerId)
          )
          .ToArray();
        ...
        var SelectedImpostor = CandidatePlayers[
          System.Random.Shared.Next(CandidatePlayers.Length)
        ].PlayerId;
    ...
    private static int GetRequiredTasksForIdentifyStep(int StepIndex)
    {
      ...
      int DefaultThreshold =
        StepIndex < DefaultMadmateTaskThresholds.Length
    ...
    void OnLightRangeUpdate(LightRangeUpdateEvent Event)
    {
      if (!AmOwner)
        return;
      if (RebelVisionMultiplier > Event.LightQuickRange)
        Event.LightQuickRange = RebelVisionMultiplier;
    }
  }
}
