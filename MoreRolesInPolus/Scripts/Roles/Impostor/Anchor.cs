...
namespace MoreRolesInPolus.Roles.Imposter
{
  ...
  public static class HarmonyPatchSetUp
  {
    ...
    public static void Preprocess(NebulaPreprocessor preprocessor)
    {
      ...
      harmony.Patch(
        typeof(Vent).GetMethod("CanUse"),
        new HarmonyMethod(typeof(HarmonyPatchSetUp).GetMethod("FixJackVentUser"), 0)
      );
    }

    public static bool FixJackVentUser(
      Vent __instance,
      ref float __result,
      ref bool canUse,
      ref bool couldUse
    )
    {
      ...
      {
        if (
          NebulaAPI.CurrentGame != null
          && GamePlayer.LocalPlayer?.Role.GetAbility<Anchor.Ability>() == null
        )
        {
          canUse = false;
          couldUse = false;
  ...
  public class Anchor : DefinedSingleAbilityRoleTemplate<Anchor.Ability>, DefinedRole, HasCitation
  {
    ...
    private Anchor()
      : base(
        "anchor",
        new(Palette.ImpostorRed),
        RoleCategory.ImpostorRole,
        NebulaTeams.ImpostorTeam,
        [placeCoolDownOption, cameraCoolDownOption, cameraDurationOption, numOfJacksOption]
      )
    {
    ...
    static readonly Image iconImage = NebulaAPI
      .AddonAsset.GetResource(string.Format("Impostor/Anchor/Anchor.png"))!
      .AsImage()!;

    /// <summary>
    /// ロビーで変更できる設定を用意します。ゲーム中で編集できるように、すぐ上のコンストラクタで役職のオプションに追加します。
    /// </summary>
    private static readonly FloatConfiguration placeCoolDownOption =
      NebulaAPI.Configurations.Configuration(
        "options.role.anchor.placeCoolDown",
        (0f, 40f, 2.5f),
        10f,
        FloatConfigurationDecorator.Second
      );
    private static readonly FloatConfiguration cameraCoolDownOption =
      NebulaAPI.Configurations.Configuration(
        "options.role.anchor.cameraCoolDown",
        (2.5f, 30f, 2.5f),
        10f,
        FloatConfigurationDecorator.Second
      );
    private static readonly FloatConfiguration cameraDurationOption =
      NebulaAPI.Configurations.Configuration(
        "options.role.anchor.cameraDuration",
        (0f, 20f, 2.5f),
        10f,
        FloatConfigurationDecorator.Second
      );
    private static readonly IntegerConfiguration numOfJacksOption =
      NebulaAPI.Configurations.Configuration("options.role.anchor.numOfJacks", (3, 5), 3);

    /// <summary>
    /// 役職の情報を用意します。
    /// </summary>
    public static readonly Anchor MyRole = new();
    ...
    public class Jack : NebulaSyncStandardObject, IGameOperator
    {
      ...
      {
        return NebulaAPI
          .AddonAsset.GetResource(
            string.Format("TricksterAnimation/trickster_box_00{0:00}.png", index + 1)
          )!
          .AsImage()!;
      }
      ...
      public Jack(Virial.Compat.Vector2 pos, bool isLocal)
        : base(pos, ZOption.Just, true, GetImage(0).GetSprite(), isLocal)
      {
        /// <summary>
      ...
      static Jack()
      {
        NebulaSyncObject.RegisterInstantiater(
          MyGlobalTag,
          (args) => new Jack(new(args[0], args[1]), false)
        );
        NebulaSyncObject.RegisterInstantiater(
          MyLocalTag,
          (args) => new Jack(new(args[0], args[1]), true)
        );
      }
    ...
    public static readonly RemoteProcess<int> RpcPlayJackVentAnimation = new(
      "RpcPlayJackVentAnimation",
      (m, _) =>
      {
    ...
    public static readonly RemoteProcess<int> RpcHideJackBeforeActivation = new(
      "RpcHideJackBeforeActivation",
      (m, _) =>
      {
        var obj = NebulaSyncObject.GetObject<Jack>(m);
        if (obj != null)
    ...
    public static readonly RemoteProcess<int[]> RpcActivateJacks = new(
      "RpcActivateJacks",
      (objectIds, _) =>
      {
    ...
    {
      private static readonly Image placeSprite = NebulaAPI
        .AddonAsset.GetResource("PlaceJackInTheBoxButton.png")!
        .AsImage(115f)!;
      private static readonly Image cameraSprite = NebulaAPI
        .AddonAsset.GetResource("PlaceJackInTheBoxButton.png")!
        .AsImage(115f)!;
      private static readonly Image moveRightSprite = NebulaAPI
        .AddonAsset.GetResource("Impostor/Anchor/ButtonRight.png")!
        .AsImage(115f)!;
      private static readonly Image moveLeftSprite = NebulaAPI
        .AddonAsset.GetResource("Impostor/Anchor/ButtonLeft.png")!
        .AsImage(115f)!;
      ...
      int[] IPlayerAbility.AbilityArguments
      {
        get
        {
          ...
          List<int> jackObjectIds =
            pendingJackObjectIds.Count > 0 ? [.. pendingJackObjectIds]
            : isJackActivated ? globalJacks?.Select(jack => jack.ObjectId).ToList() ?? []
      ...
      public Ability(
        GamePlayer player,
        bool isUsurped,
        int initialLeftJacks,
        bool isJackActivated,
        int[] jackObjectIds
      )
        : base(player, isUsurped)
      {
        ...
        if (AmOwner)
        {
          ...
          var placeButton = NebulaAPI
            .Modules.AbilityButton(
              this,
              MyPlayer,
          ...
          placeButton.OnClick = (button) =>
          {
            ...
            var hits = UnityEngine.Physics2D.OverlapCircleAll(
              playerPos,
              0.6f,
              UnityEngine.LayerMask.GetMask("Ship", "Decls")
            );
            ...
            var obj = NebulaSyncObject.LocalInstantiate(
              Jack.MyLocalTag,
              new float[] { finalPos.x, finalPos.y }
            );
          ...
          var cameraButton = NebulaAPI
            .Modules.EffectButton(
              this,
              MyPlayer,
              Virial.Compat.VirtualKeyInput.SecondaryAbility,
              cameraCoolDownOption,
              cameraDurationOption,
              "camera",
              cameraSprite,
              null,
              _ =>
          ...
          {
            TryRestoreJacksFromPendingIds();
            if (globalJacks == null || globalJacks.Count == 0)
              return;
          ...
          {
            if (globalJacks == null || globalJacks.Count == 0 || !isInCameraMode)
              return;
          ...
          var nextCameraButton = NebulaAPI.Modules.AbilityButton(
            this,
            MyPlayer,
            Virial.Compat.VirtualKeyInput.None,
            0f,
            "nextCamera",
            moveRightSprite,
            null,
            _ => isInCameraMode && globalJacks != null && globalJacks.Count > 1
          );
          ...
          {
            currentCameraIndex++;
            if (currentCameraIndex >= globalJacks.Count)
              currentCameraIndex = 0;
            UpdateCameraTarget();
          };

          // 前のカメラボタン（カメラモード中のみ表示）- 右側に表示
          var prevCameraButton = NebulaAPI.Modules.AbilityButton(
            this,
            MyPlayer,
            Virial.Compat.VirtualKeyInput.None,
            0f,
          ...
          prevCameraButton.OnClick = (button) =>
          {
            ...
            if (currentCameraIndex < 0)
              currentCameraIndex = globalJacks.Count - 1;
            UpdateCameraTarget();
          };
          ...
          GameOperatorManager.Instance.Subscribe<GameUpdateEvent>(
            ev =>
            {
              if (!isInCameraMode || globalJacks == null || globalJacks.Count == 0)
                return;
              ...
              if (PlayerControl.LocalPlayer.lightSource != null)
              {
                ...
                PlayerControl.LocalPlayer.lightSource.transform.position =
                  new Virial.Compat.Vector3(
                    camPos.x,
                    camPos.y,
                    PlayerControl.LocalPlayer.lightSource.transform.position.z
                  );
              }
          ...
          GameOperatorManager.Instance.Subscribe<PlayerVentEnterEvent>(
            ev =>
            {
          ...
          GameOperatorManager.Instance.Subscribe<PlayerVentExitEvent>(
            ev =>
            {
              if (ev.Player.AmOwner)
              {
          ...
          GameOperatorManager.Instance.Subscribe<MeetingStartEvent>(
            ev =>
            {
              // 会議開始時にカメラモードを終了
              if (isInCameraMode)
      ...
      {
        if (pendingJackObjectIds.Count == 0)
          return;
        ...
        foreach (int objectId in pendingJackObjectIds)
        {
          ...
          if (restoredJack == null)
            return;
          restoredJacks.Add(restoredJack);
        }

        if (isJackActivated)
          globalJacks = restoredJacks;
        else
          localJacks = restoredJacks;
        pendingJackObjectIds.Clear();
      }

      private void PromoteLocalJacksForInheritance()
      {
        if (localJacks == null || localJacks.Count == 0)
          return;
